  ...
  return (
    <main className="flex min-h-screen flex-col justify-between p-24 container mx-auto px-1 lg:px-20 md:px-10">
        ...
        {myJobs.map((post:any)=>{
            ...
            return(
                <div className="jobCard border-solid border-2 py-7 px-7 rounded-2xl" key={jobID}>
                <p className="text-2xl font-medium hover:text-red-700 py-0">{title}</p>
                <p className="text-gray-400 py-2">Posted on: {date}</p>
                <div className="py-6 flex justify-between">
                <p>Budget: <span className="text-gray-400">{budget} TK</span></p>
                <p>Duration: <span className="text-gray-400">{duration}</span></p>
                </div>
                <button className="text-white bg-red-700 px-3 py-1 rounded-xl hover:bg-black">See more</button>
                </div>
            )
        })}
         </div>
