...
import jobImg from "../../../assets/postJob.png";
...
export default function AdminJob() {
  ...
  const deleteJob = async (jobID:any) => {
    try {
        ...
        await axios.delete(`http://localhost:8000/admin/removeJobs/${jobID}`, config);
        console.log("Removed");
        router.push("/adminPages/job");
    } catch (error) {
      ...
      <main className="flex min-h-screen flex-col justify-between p-24 container mx-auto px-1 lg:px-20 md:px-10">
        <div className="flex justify-between items-center mb-12 bg-red-100 p-8 rounded-2xl">
          <div>
            ...
            <Link
              className="bg-red-700 text-white text-xl px-16 py-3 mt-6 rounded-full hover:bg-black hover:text-white"
              href="/job/jobPost"
            >
            ...
            return (
              ...
              >
                ...
                <div className="py-6 flex justify-between">
                  ...
                  <button
                    className="text-white bg-red-700 mx-3 px-3 py-1 rounded-xl hover:bg-black"
                    onClick={() => deleteJob(jobID)}
                  >
                    Delete
                  </button>
                  <Link
                    className="text-white bg-blue-600 px-3 py-1 rounded-xl hover:bg-black"
                    href={{
                      pathname: "/adminPages/job/job-details",
                      query: {
                        JobID: jobID,
                        Title: title,
